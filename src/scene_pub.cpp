...
void collisionObjects(moveit::planning_interface::PlanningSceneInterface& planning_scene_interface) {
    ...
    // Camera collision 
    collision_objects[0].id = "camera_link";
    ...
    collision_objects[1].primitives[0].dimensions[2] = 0.016;

    /* Define the pose of the table. */
    collision_objects[1].primitive_poses.resize(1);
    collision_objects[1].primitive_poses[0].position.x = 0.860;
    collision_objects[1].primitive_poses[0].position.y = -0.009;
...
int main(int argc, char **argv) {
    ...
    ROS_INFO_STREAM("Placing collision object for depth camera and table");
